...
```{r class_pwc}
# Pairwise comparison between fuel load by thinning treatment, for each fuel class 
# We analyzed the effect of thinning treatment for each fuel class. The Bonferroni adjustment was applied. 
# A post hoc pairwise comparison showed mean fuel load was significantly different between control and treatment (thinned and unthinned, pre- and post-thinning) measurements for the 1000-hr sound (p-adj. < 0.001) and 10-hr (p-adj. < 0.01) fuel classes. No significant effect was found for 1-hr (p-adj. = 0.131), 100-hr (p-adj. = 0.402), or 1000-hr rotten (p-adj. = 0.347).

wd_transform_class  %>%
  group_by(fuel_class) %>%
    fxn_pwc(index_value = "value_tran",
             index_id = "plot_id",
             index_time = "timing") %>%
  # fxn_aov_me(index_value = "value_tran",
  #            index_id = "plot_id",
