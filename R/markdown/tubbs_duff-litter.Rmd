...
First, we caclulated the plot-level mean (n = 4 quadrats) for each fuel class (duff, litter) at each time point. A subset of the data frame with the plot-level mean for each fuel class by year is shown below.  
...
A boxplot of total duff and litter by year showed lower values in post-fire years. Specifically, we observed a decrease in the depth of all duff and litter immediately following the fire (2017; 0-y post-fire). The mean depth of duff and litter did not return to pre-fire levels in subsequent years.
...
We found a significant difference in mean depth betwen duff and litter in 2019 and in 2021.
