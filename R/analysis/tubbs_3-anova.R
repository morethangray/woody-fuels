...
# we now have additional fuels data for forests that were burned in Tubbs and recently tubbsned
# It will not be a repeated measures test because we only have two time steps. 
# The key tubbsg to account for is that our new forest plots have three transects per plot instead of just two 
...
dl_transform <- 
  read_csv(here(path_derived, "tubbs_dl_transformed_metric-units.csv")) %>%
...
dl_transform_class %>%
  ...
  select(data_type, 
         ...
         starts_with("d_")) %>%
  fxn_kable()
# Significant difference in litter between pre- and post-tubbsning
...
# 100-hr: notubbsg looks great, maybe ordernorm?
# 1000-hr rotten: notubbsg looks great, maybe sqrt? maybe ordernorm?
...
# We conducted post hoc pairwise comparisons between the levels of the witubbs-subjects factor (here, timing). The result of paired t-tests between timings showed no significant difference in fuel load between timings at a significance level of \< 0.05. This finding is consistent with the lack of main effect found for timing.
...
aov_me <- 
  input_total_transform %>%
  anova_test(dv = value, wid = plot_id, witubbs = timing)
